...
class PlayState extends MusicBeatState
{
	...
	public function startVideo(name:String):Void
	{
		...
		var fileName:String = Paths.modFolders('videos/' + name + '.' + Paths.VIDEO_EXT);
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				if (!Std.isOfType(songNotes[3], String))
					swagNote.noteType = fnf.editors.ChartingState.noteTypeList[songNotes[3]]; // Backward compatibility + compatibility with Week 7 charts
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		for (c in Std.string(combo).split(''))
		{
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null):String
	{
		...
		for (i in 0...achievementsToCheck.length)
		{
			...
			{ // Requires jsons for call
				...
				if (!Achievements.isAchievementUnlocked(json.icon) && json.customGoal && !unlock)
				{
					unlock = true;
					achievementName = json.icon;
				}
